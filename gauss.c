...
int main(){
	...
	//Повышаем контрастностью. Светлее 170, делаем белым. Все, что темнее 70, делаем черным
	for(i=0; i<size; i++){
		if(idata[i]>170)
			idata[i] = 255;
		else if(idata[i]<70)
	...
	//Повышаем контрастностью. Светлее 120, делаем белым. Все, что темнее 70, делаем черным. В другом случае - серым
	for(i=0; i<size; i++){
		if(odata2[i]>120)
			odata2[i] = 255;
		else if(odata2[i]<70)
